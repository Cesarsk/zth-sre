...
test('index supports discovery, clear availability and a guided workspace', async ({ page }) => {
  ...
  await expect(page.getByText('Available exercise', { exact: true })).toHaveCount(11);
  ...
  const answer = page.getByRole('tooltip').first();
  await expect(answer).toBeHidden();
  await page.getByRole('button', { name: /What evidence tells you the dependency was involved/ }).hover();
...
test('incident exercise briefs can start a deterministic lab', async ({ page }) => {
  ...
  await expect(page.getByText('outcomes', { exact: false }).first()).toBeVisible();
