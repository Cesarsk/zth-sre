...
const scenarioExerciseIDs = ['cpu-saturation', 'useful-alerts', 'slo-burn-rate', 'vertical-horizontal', 'dependency-bottleneck', 'connection-pool', 'latency-slo', 'dns-failure', 'retry-storm', 'memory-leak', 'autoscaler-oscillation'];
...
type ScenarioCatalogItem = { id: string; diagram?: DiagramData; hints?: string[] };
...
export function App() {
  ...
  const [grade, setGrade] = useState<{ passed?: boolean; feedback?: string; availability?: number; p95_latency_ms?: number; requests?: number } | null>(null);
  ...
  const filtered = exercises.filter(item => `${item.title} ${item.category} ${item.summary}`.toLowerCase().includes(search.toLowerCase().trim()));
  ...
  useEffect(() => {
    if (!exerciseMode) { setHistory([]); return; }
    let stopped = false;
    const poll = async () => {
      try {
  ...
  const runtimeAction = async (path: string, options?: RequestInit) => {
    ...
    try {
      ...
      if (data.state || data.scenario) setRun(data);
  ...
  return (
    <main>
      <header className="page-header">
        <div><p className="eyebrow">Local reliability workspace</p><h1>SRE Lab</h1></div>
      ...
      {isIndex ? <section className="catalog" aria-labelledby="catalog-heading">
        ...
        <h2 id="catalog-heading">Exercise index</h2>
        <p className="intro">Start with the guided walkthrough to learn your way around the lab. Each exercise has its own brief, objectives and investigation workspace.</p>
         <p className="phase-note">The walkthrough and eleven deterministic incident exercises are available. Choose an exercise to read its runbook before starting it.</p>
        ...
        <nav className="breadcrumb" aria-label="Breadcrumb"><a href="#/">Exercise index</a><span>/ Guided walkthrough</span></nav>
        <section className="exercise-brief" aria-labelledby="exercise-heading">
          ...
          <ul className="learning-objectives">{exercise.learningObjectives?.map(item => <li key={item}>{item}</li>)}</ul>
           {!walkthrough && <ResourceDiagram id={exercise.id} active={activeRun} phase={run.phase} diagram={scenarioCatalog.find(item => item.id === exercise.id)?.diagram} />}
          {walkthrough ? <p className="phase-note">No automatic grade: mark each step after checking its expected result. Your self-check saves automatically in this local lab and is restored when you return. `make reset` clears saved progress with the lab baseline.</p>
            : <div className="runtime-controls"><p className="phase-note"><strong>Executable exercise.</strong> Start the deterministic traffic and fault scenario, investigate with the terminal, then check observable outcomes.</p><div className="button-row">{!activeRun ? <button className="primary-button" onClick={startExercise}>Start exercise</button> : <><button className="secondary-button" onClick={resetExercise}>Reset exercise</button><button className="primary-button" onClick={checkExercise}>Check solution</button></>}</div>{runMessage && <p role="alert" className="error">{runMessage}</p>}{activeRun && <p className="run-status">Run <code>{run.runID}</code> / phase <strong>{run.phase}</strong></p>}</div>}
        </section>
      </>}

      <div className={exercise ? 'exercise-workspace' : ''}>
      {exercise && <aside className="instructions" aria-labelledby="instructions-heading">
        ...
        <ol className="commands">{exercise.steps?.map((step, index) => <li key={step.title}>
          ...
          {walkthrough && 'command' in step && <pre><code>{step.command}</code></pre>}
          <p className="expected"><strong>Expected result:</strong> {step.expected}</p>
          <div className="think-about"><strong>Think about:</strong><button type="button" aria-describedby={`answer-${exercise.id}-${index}`}>{step.question}<span className="answer-cue">Hover for an answer</span></button><span id={`answer-${exercise.id}-${index}`} role="tooltip" className="answer-tooltip"><strong>Answer:</strong> {step.answer}</span></div>
        ...
        {walkthrough && <><label className="notes-label" htmlFor={`notes-${exercise.id}`}>Investigation notes<textarea id={`notes-${exercise.id}`} maxLength={16000} disabled={!progressReady} value={notes} onChange={event => updateNotes(event.target.value)} placeholder="Save observations, hypotheses, commands and next steps here." /></label><button className="secondary-button" disabled={!progressReady} onClick={() => updateChecklist([])}>Clear checklist</button><p className={`save-state ${progressState}`}>{progressState === 'saving' ? 'Saving progress...' : progressState === 'saved' ? 'Progress saved locally.' : progressState === 'error' ? 'Progress could not be saved. Your current work is still visible.' : ''}</p><p className="phase-note">If a request fails, read the HTTP status and check the service health on the right. If metrics are empty, make a request and wait for a scrape. Nothing here requires scaling or fixing an injected fault.</p></>}
         {exerciseMode && <><h3 className="hints-heading">Hints</h3><ol className="hint-list">{exercise.hints?.map((hint, index) => <li key={hint}>{run?.revealedHints?.includes(index) ? hint : 'Hint available'}</li>)}</ol>{activeRun && <div className="button-row"><button className="secondary-button" onClick={() => void runtimeAction('/api/run/hint', { method: 'POST', body: '{}' })}>Reveal next hint</button>{['cpu-saturation', 'vertical-horizontal'].includes(exercise.id) && <button className="secondary-button" onClick={() => void runtimeAction('/api/run/capacity', { method: 'POST', body: JSON.stringify({ activeBackends: 2 }) })}>Activate API capacity</button>}{(exercise.id === 'useful-alerts' || exercise.id === 'slo-burn-rate') && <><button className="secondary-button" onClick={() => setPhase('incident')}>Inject incident phase</button><button className="secondary-button" onClick={() => setPhase('recovery')}>Start recovery phase</button></>}{interventionLabels[exercise.id] && <button className="secondary-button" onClick={() => void runtimeAction('/api/run/intervention', { method: 'POST', body: JSON.stringify({ action: interventionLabels[exercise.id].action }) })}>{interventionLabels[exercise.id].label}</button>}</div>}{(exercise.id === 'useful-alerts' || exercise.id === 'slo-burn-rate') && activeRun && <label className="notes-label">PromQL alert expression<textarea value={alertExpression} onChange={event => setAlertExpression(event.target.value)} placeholder="Enter a read-only PromQL alert expression" /><button className="secondary-button" onClick={saveAlert}>Save alert expression</button></label>}</>}
         {grade && <div className={`grade ${grade.passed ? 'pass' : 'fail'}`} role="status"><strong>{grade.passed ? 'Solution check passed' : 'Solution check needs more evidence'}</strong><p>{grade.feedback}</p>{grade.availability !== undefined && <small>Availability {((grade.availability ?? 0) * 100).toFixed(2)}% / p95 {Math.round(grade.p95_latency_ms ?? 0)} ms / {Math.round(grade.requests ?? 0)} requests</small>}</div>}
         {exerciseMode && history.filter(item => item.scenario === exercise.id).length > 0 && <section className="run-history" aria-labelledby="history-heading"><h3 id="history-heading">Recent checks</h3><ul>{history.filter(item => item.scenario === exercise.id).slice(-5).reverse().map(item => <li key={`${item.runID}-${item.checkedAt}`}><span>{item.passed ? 'Passed' : 'Needs evidence'}</span><time dateTime={item.checkedAt}>{new Date(item.checkedAt).toLocaleString()}</time>{item.p95LatencyMS !== undefined && <small>{Math.round(item.p95LatencyMS)} ms p95</small>}</li>)}</ul></section>}
      </aside>}
      <div className="lab-workspace">
       <section className="overview" aria-labelledby="system-heading">
        ...
        <p className="phase-note">Real containers, generated traffic and Prometheus evidence drive the exercise. A solution check evaluates observable outcomes, not a specific command.</p>
      ...
      <footer>One local lab. Real processes and metrics. No simulated exercise results.</footer>
    </main>
  );
}
