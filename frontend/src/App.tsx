...
type RunHistory = { runID: string; scenario: string; checkedAt: string; passed: boolean; availability?: number; p95LatencyMS?: number; score?: number };
type ScenarioCatalogItem = { id: string; diagram?: DiagramData; hints?: string[]; goal?: string; successCriteria?: string[]; prerequisites?: string[]; requiredTools?: string[]; falseHypotheses?: string[] };
type LearnerProfile = { xp: number; rank: string; nextRank?: string; nextRankXP?: number; completedIDs: string[]; bestScores: Record<string, number>; badges: { id: string; title: string; description: string; earnedAt: string }[] };
type Grade = { passed?: boolean; feedback?: string; availability?: number; p95_latency_ms?: number; requests?: number; objective_availability?: number; objective_p95_latency_ms?: number; minimum_requests?: number; diagnosisRecorded?: boolean; diagnosisCorrect?: boolean; recoverySeen?: boolean; alertBaseline?: boolean; alertFired?: boolean; alertCleared?: boolean; interventions?: string[]; score?: number; scoreBreakdown?: { evidence: number; mitigation: number; verification: number; precision: number }; xpAwarded?: number; profile?: LearnerProfile; progressionError?: string; progressionNote?: string };
...
function parseProfile(value: unknown): LearnerProfile {
  const data = value as Partial<LearnerProfile> | null;
  return { xp: typeof data?.xp === 'number' ? data.xp : 0, rank: typeof data?.rank === 'string' ? data.rank : 'Observer', nextRank: typeof data?.nextRank === 'string' ? data.nextRank : undefined, nextRankXP: typeof data?.nextRankXP === 'number' ? data.nextRankXP : undefined, completedIDs: Array.isArray(data?.completedIDs) ? data.completedIDs : [], bestScores: data?.bestScores && typeof data.bestScores === 'object' ? data.bestScores as Record<string, number> : {}, badges: Array.isArray(data?.badges) ? data.badges : [] };
}
...
export function App() {
  ...
  const nextMission = learningPath.map(id => exercises.find(item => item.id === id)).find(item => item && !completedIDs.includes(item.id));
  const featuredMission = exercises[1 + Math.floor(Date.now() / 604800000) % scenarioExerciseIDs.length];
  const skillTracks = [{ name: 'Capacity', ids: ['cpu-saturation', 'vertical-horizontal', 'autoscaler-oscillation'] }, { name: 'Reliability', ids: ['useful-alerts', 'slo-burn-rate', 'latency-slo', 'retry-storm'] }, { name: 'Systems', ids: ['dependency-bottleneck', 'connection-pool', 'memory-leak', 'file-forensics'] }, { name: 'Networking', ids: ['dns-failure', 'blocked-traffic'] }];
  ...
  useEffect(() => {
    let stopped = false;
    const load = () => fetch('/api/profile', { cache: 'no-store' })
      .then(response => response.ok ? response.json() as Promise<unknown> : null)
      .then(data => { if (!stopped && data) setProfile(parseProfile(data)); })
      .catch(() => undefined);
    void load();
  ...
  useEffect(() => {
    if (!activeRun || !run.startedAt) { setElapsed(0); return; }
    const update = () => setElapsed(Math.max(0, Math.floor((Date.now() - new Date(run.startedAt ?? '').getTime()) / 1000)));
    update();
  ...
  const checkExercise = async () => { const data = await runtimeAction('/api/run/check', { method: 'POST', body: '{}' }); if (data) { setGrade(data); if (data.profile) setProfile(data.profile); } };
    ...
    <main id="main-content" aria-busy={pending}>
      ...
      <header className="page-header">
       ...
       {isIndex ? <section className="catalog" aria-labelledby="catalog-heading">
         ...
         <section className="mission-board" aria-labelledby="mission-board-heading"><div className="mission-board-title"><div><p className="eyebrow">Command center</p><h2 id="mission-board-heading">Your on-call progression</h2></div><span className="rank-chip">{profile?.rank ?? 'Observer'}</span></div><div className="mission-board-grid"><article><span>Mission XP</span><strong>{profile?.xp ?? 0}</strong>{profile?.nextRank && <small>{Math.max(0, (profile.nextRankXP ?? 0) - (profile.xp ?? 0))} XP to {profile.nextRank}</small>}</article><article><span>Resolved incidents</span><strong>{completedIDs.length}<small> / {scenarioExerciseIDs.length}</small></strong><small>Verified outcomes, not clicks</small></article><article><span>Next dispatch</span><strong className="next-dispatch">{nextMission?.title ?? 'Campaign complete'}</strong>{nextMission ? <a href={`#/exercises/${nextMission.id}`}>Open mission →</a> : <small>Replay any mission to improve your score.</small>}</article></div><div className="skill-tracks" aria-label="Skill tracks">{skillTracks.map(track => <div key={track.name}><span>{track.name}</span><strong>{track.ids.filter(id => completedIDs.includes(id)).length}/{track.ids.length}</strong></div>)}</div><p className="weekly-drill"><span>Weekly drill</span><a href={`#/exercises/${featuredMission.id}`}>{featuredMission.title} · on-call scenario →</a></p>{profile?.badges.length ? <div className="badge-shelf" aria-label="Earned badges">{profile.badges.map(badge => <span key={badge.id} title={badge.description}>◆ {badge.title}</span>)}</div> : <p className="mission-board-empty">Earn badges by recording evidence, applying a narrow fix, and proving recovery under load.</p>}</section>
        ...
        {categories.map(category => <section className="catalog-category" aria-labelledby={`category-${category}`} key={category}><div className="category-heading"><h3 id={`category-${category}`}>{category}</h3><span>{filtered.filter(item => item.category === category).length} exercises</span></div><div className="exercise-grid">{filtered.filter(item => item.category === category).map(item => <article className={`exercise-card ${item.status}`} key={item.id}>
          ...
          <h3>{item.title}</h3><p>{item.summary}</p>
           {completedIDs.includes(item.id) && <span className="completion-badge">✓ Mission resolved · {profile?.rank ?? 'Observer'} progression</span>}
           <div className="card-action"><span>{item.duration}{profile?.bestScores[item.id] ? ` · best ${profile.bestScores[item.id]}/100` : ''}{completedIDs.includes(item.id) ? ' · replay available' : ''}</span><a className={item.status === 'available' ? 'primary-link' : 'preview-link'} href={`#/exercises/${item.id}`}>{item.status === 'available' ? (item.id === 'first-investigation' ? 'Start walkthrough' : 'Open exercise') : 'View planned brief'}</a></div>
        </article>)}</div></section>)}
        ...
        <section className="exercise-brief" aria-labelledby="exercise-heading">
          ...
          {!walkthrough && <section className="goal-contract"><p className="eyebrow">Investigation contract</p><h3>{selectedScenario?.goal ?? exercise.objective}</h3><p>Choose the explanation your evidence supports. The lab accepts a mitigation only after you record a diagnosis.</p><ul>{selectedScenario?.successCriteria?.map(criteria => <li key={criteria}>{criteria}</li>)}</ul><label>My diagnosis<select value={diagnosis} onChange={event => setDiagnosis(event.target.value)}><option value="">Select the hypothesis your evidence supports</option>{diagnosisOptions[exercise.id]?.map(option => <option value={option.value} key={option.value}>{option.label}</option>)}</select></label><label>Evidence note<textarea value={evidenceNote} minLength={10} maxLength={2000} onChange={event => setEvidenceNote(event.target.value)} placeholder="What did you observe, and why does it support this diagnosis?" /></label><button className="primary-button" disabled={!activeRun || !diagnosis || evidenceNote.trim().length < 10 || pending} onClick={() => void submitDiagnosis()}>{diagnosisRecorded ? 'Update diagnosis' : 'Record diagnosis'}</button>{diagnosisRecorded && <p className="action-message">Diagnosis recorded. You can now test a mitigation and verify the goal.</p>}</section>}
           ...
           {!walkthrough && selectedScenario && <dl className="exercise-metadata">{selectedScenario.prerequisites?.length ? <div><dt>Prerequisites</dt><dd>{selectedScenario.prerequisites.join(' · ')}</dd></div> : null}{selectedScenario.requiredTools?.length ? <div><dt>Tools</dt><dd>{selectedScenario.requiredTools.join(' · ')}</dd></div> : null}{selectedScenario.falseHypotheses?.length ? <div><dt>Do not assume</dt><dd>{selectedScenario.falseHypotheses.join(' · ')}</dd></div> : null}</dl>}
      ...
      {exerciseMode && activeRun && <><section className="mission-hud" aria-label="Active incident status"><div><span>Active incident</span><strong>{exercise?.title}</strong></div><div><span>Elapsed</span><strong>{Math.floor(elapsed / 60)}:{String(elapsed % 60).padStart(2, '0')}</strong></div><div><span>Phase</span><strong>{run.phase}</strong></div><div><span>System</span><strong>{label}</strong></div><div><span>Evidence</span><strong>{run.evidenceNote ? 'Recorded' : 'Collecting'}</strong></div><div><span>Diagnosis</span><strong>{diagnosisRecorded ? 'Evidence-backed' : 'Open'}</strong></div><div><span>Recovery</span><strong>{grade?.passed ? 'Confirmed' : run.phase === 'recovery' ? 'Verifying' : 'Not started'}</strong></div></section><ol className="mission-timeline" aria-label="Incident timeline"><li className="complete"><span>01</span>Briefed</li><li className={run.phase === 'incident' || run.phase === 'recovery' ? 'complete' : ''}><span>02</span>Incident observed</li><li className={run.evidenceNote ? 'complete' : ''}><span>03</span>Evidence collected</li><li className={diagnosisRecorded ? 'complete' : ''}><span>04</span>Diagnosis recorded</li><li className={(run.interventions?.length ?? 0) > 0 ? 'complete' : ''}><span>05</span>Mitigation applied</li><li className={grade?.passed ? 'complete' : ''}><span>06</span>Recovery confirmed</li></ol></>}
      ...
      {exercise && <aside className="instructions" aria-labelledby="instructions-heading">
        ...
        {walkthrough && <><label className="notes-label" htmlFor={`notes-${exercise.id}`}>Investigation notes<textarea id={`notes-${exercise.id}`} maxLength={16000} disabled={!progressReady} value={notes} onChange={event => updateNotes(event.target.value)} placeholder="Save observations, hypotheses, commands and next steps here." /></label><button className="secondary-button" disabled={!progressReady} onClick={() => updateChecklist([])}>Clear checklist</button><p className={`save-state ${progressState}`}>{progressState === 'saving' ? 'Saving progress...' : progressState === 'saved' ? 'Progress saved locally.' : progressState === 'error' ? 'Progress could not be saved. Your current work is still visible.' : ''}</p><p className="phase-note">If a request fails, read the HTTP status and check the service health on the right. If metrics are empty, make a request and wait for a scrape. Nothing here requires scaling or fixing an injected fault.</p></>}
          {exerciseMode && <><h3 className="hints-heading">Hints</h3><ol className="hint-list">{exercise.hints?.map((hint, index) => <li key={hint}>{run?.revealedHints?.includes(index) ? hint : 'Hint available'}</li>)}</ol>{activeRun && <div className="button-row"><button className="secondary-button" onClick={() => void runtimeAction('/api/run/hint', { method: 'POST', body: '{}' })}>Reveal next hint</button>{exercise.id === 'file-forensics' && <button className="secondary-button" disabled={!diagnosisRecorded || pending} onClick={() => void runtimeAction('/api/run/file-evidence', { method: 'POST', body: '{}' })}>Mark lsof evidence found</button>}{['cpu-saturation', 'vertical-horizontal'].includes(exercise.id) && <button className="secondary-button" disabled={!diagnosisRecorded || pending} onClick={() => void runtimeAction('/api/run/capacity', { method: 'POST', body: JSON.stringify({ activeBackends: 2 }) })}>Apply capacity change</button>}{(exercise.id === 'useful-alerts' || exercise.id === 'slo-burn-rate') && <><button className="secondary-button" onClick={() => setPhase('incident')}>Inject incident phase</button><button className="secondary-button" disabled={!diagnosisRecorded || pending} onClick={() => setPhase('recovery')}>Start recovery phase</button></>}{interventionLabels[exercise.id] && <button className="secondary-button" disabled={!diagnosisRecorded || pending} onClick={() => void runtimeAction('/api/run/intervention', { method: 'POST', body: JSON.stringify({ action: interventionLabels[exercise.id].action }) })}>{interventionLabels[exercise.id].label}</button>}</div>}{(exercise.id === 'useful-alerts' || exercise.id === 'slo-burn-rate') && activeRun && <label className="notes-label">PromQL alert expression<textarea value={alertExpression} onChange={event => setAlertExpression(event.target.value)} placeholder="Enter a read-only PromQL alert expression" /><button className="secondary-button" disabled={!diagnosisRecorded || pending} onClick={saveAlert}>Save alert expression</button></label>}</>}
           {grade && <div className={`grade ${grade.passed ? 'pass' : 'fail'}`} role="status"><strong>{selfCheck ? (grade.passed ? 'Self-check recorded' : 'Record your investigation first') : grade.passed ? 'Mission resolved' : 'Solution check needs more evidence'}</strong><p>{selfCheck ? 'This records your confirmation. The lab does not automatically inspect your lsof evidence.' : grade.feedback}</p>{!selfCheck && grade.availability !== undefined && <small>Availability {((grade.availability ?? 0) * 100).toFixed(2)}% / p95 {Math.round(grade.p95_latency_ms ?? 0)} ms / {Math.round(grade.requests ?? 0)} requests</small>}{!selfCheck && <dl className="grade-evidence"><div><dt>Diagnosis</dt><dd>{grade.diagnosisCorrect ? 'Correct' : grade.diagnosisRecorded ? 'Recorded, not supported' : 'Not recorded'}</dd></div><div><dt>Recovery</dt><dd>{grade.recoverySeen ? 'Observed' : 'Not observed'}</dd></div>{(exercise.id === 'useful-alerts' || exercise.id === 'slo-burn-rate') && <div><dt>Alert lifecycle</dt><dd>{grade.alertBaseline ? 'Baseline fired' : 'Quiet baseline'} / {grade.alertFired ? 'Fired' : 'Did not fire'} / {grade.alertCleared ? 'Cleared' : 'Not cleared'}</dd></div>}</dl>}{grade.passed && !selfCheck && grade.scoreBreakdown && <section className="incident-debrief"><div><p className="eyebrow">Incident debrief</p><h3>Score {grade.score ?? 0} / 100 {grade.xpAwarded ? `· +${grade.xpAwarded} XP` : ''}</h3></div><dl><div><dt>Evidence</dt><dd>{grade.scoreBreakdown.evidence}/25</dd></div><div><dt>Mitigation</dt><dd>{grade.scoreBreakdown.mitigation}/25</dd></div><div><dt>Verification</dt><dd>{grade.scoreBreakdown.verification}/25</dd></div><div><dt>Precision</dt><dd>{grade.scoreBreakdown.precision}/25</dd></div></dl>{grade.profile && <p>{grade.profile.rank} rank · {grade.profile.xp} total XP. Replays earn only an improvement over the saved best score.</p>}{grade.progressionNote && <p>{grade.progressionNote}</p>}{grade.progressionError && <p className="error">{grade.progressionError}</p>}</section>}</div>}
          ...
          {(grade?.passed || (walkthrough && progressReady && checked.length === exercise.steps?.length)) && <section className="completion-panel"><p className="eyebrow">Keep building your skills</p><h3>Take the learning with you.</h3><ul>{exercise.learningObjectives?.map(skill => <li key={skill}>{skill}</li>)}</ul><p>Before moving on, note the evidence that changed your hypothesis.{activeRun && ' Reset this run to leave the lab ready for your next exercise.'}</p>{nextExercise && <a className="primary-link" href={`#/exercises/${nextExercise.id}`}>Next: {nextExercise.title} →</a>}<a href="#/">Explore the exercise library</a></section>}
           {exerciseMode && history.filter(item => item.scenario === exercise.id).length > 0 && <section className="run-history" aria-labelledby="history-heading"><h3 id="history-heading">Recent checks</h3><p className="phase-note">Replay this mission to improve your evidence and precision score.</p><p><a className="secondary-button" href="/api/runs/export">Export run history</a></p><ul>{history.filter(item => item.scenario === exercise.id).slice(-5).reverse().map(item => <li key={`${item.runID}-${item.checkedAt}`}><span>{item.passed ? (selfCheck ? 'Self-check recorded' : 'Passed') : 'Needs evidence'}</span><time dateTime={item.checkedAt}>{new Date(item.checkedAt).toLocaleString()}</time>{item.score ? <small>{item.score}/100</small> : !selfCheck && item.p95LatencyMS !== undefined && <small>{Math.round(item.p95LatencyMS)} ms p95</small>}</li>)}</ul></section>}
      </aside>}
