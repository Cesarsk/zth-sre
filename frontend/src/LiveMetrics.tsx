...
type Values = { requestRate?: number; errorRate?: number; p95?: number; cpu?: number };
...
export function LiveMetrics({ runID, active }: Props) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      const [requestRate, errorRate, p95, cpu] = await Promise.all([
      ...
      if (!stopped) setMetrics({ requestRate, errorRate, p95: p95 === undefined ? undefined : p95 * 1000, cpu });
    };
    void refresh();
